...
- Log only information that helps a restarted agent continue: goal, current understanding, key decisions, assumptions to verify, touched files, verification status, blockers, and next steps.
...
- Keep `context.md` under `20,000` bytes and `200` lines; while it stays manageable, keep deeper analysis there, and when it grows too large, split durable detail into `.zero-memory/context/<task_slug>/references/` and use `skills/zero-context-persistence/scripts/compact_context.py`.
...
- Force rule for memory recall:
 - When the agent believes prior experience may matter for the current task, it MUST use `skills/zero-memory-curator/SKILL.md`.
 - When the agent encounters a problem, failure, blocker, confusing behavior, or repeated debugging loop, it MUST also use `skills/zero-memory-curator/SKILL.md` to check whether prior experience may help solve it.
 - First run `python3 skills/zero-memory-curator/scripts/load_memory_graph.py --root .zero-memory/memory --depth 0` to load description-first summaries from the init set; if the task already has a strong anchor, use `--start <memory-id>` instead of opening memory files directly.

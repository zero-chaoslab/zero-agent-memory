...
function filteredNodes() {
    ...
    return DATA.nodes.filter(node => {
        if (state.layer !== 'all' && node.layer !== state.layer)
            return false;
        if (state.status !== 'all' && node.status !== state.status)
            return false;
        if (query && !searchable(node).includes(query))
            return false;
        return true;
    });
}
...
function selectedRelationshipIds(selected) {
    ...
    for (const node of DATA.nodes) {
        if ((node.related || []).includes(selected.id))
            ids.add(node.id);
        if ((node.load_next || []).includes(selected.id))
            ids.add(node.id);
    }
    return ids;
}
...
function graphNodes(nodes) {
    ...
    if (state.graphMode === 'drilldown')
        return drilldownNodes(nodes);
    const limit = Math.max(12, Math.min(state.nodeLimit, nodes.length));
...
function graphEdges(nodeSet) {
    ...
    const selectedEdges = state.selectedId
        ...
        : [];
    if (state.graphMode === 'drilldown' && state.selectedId)
...
function nodeColor(node) {
    if (metric(node, 'missed_recall_count') > 0)
        return getCss('--missed');
    if (metric(node, 'helpful_count') > 0)
        return getCss('--helpful');
    if (metric(node, 'hotness_score') > 0 || metric(node, 'recall_count') > 0)
        return getCss('--hot');
    return getCss('--quiet');
}
...
function layout(nodes, width, height) {
    if (state.graphMode === 'init')
        return layoutInit(nodes, width, height);
    if (state.graphMode === 'drilldown')
        return layoutDrilldown(nodes, width, height);
...
function renderTable(nodes) {
    ...
    tbody.querySelectorAll('tr[data-id]').forEach(row => {
        row.addEventListener('click', () => selectMemory(row.dataset.id || ''));
    });
}
...
function fileUrlFor(value) {
    ...
    if (generatedFileLinks[repoPath])
        return generatedFileLinks[repoPath];
    const generatedRawLinks = DATA.metadata.file_raw_links || {};
    if (generatedRawLinks[repoPath])
        return generatedRawLinks[repoPath];
...
async function previewFile(rawHref, path, previewHref = '') {
    ...
    try {
        ...
        setFilePreview(path, rawHref, previewHref, `<pre>Could not load this file in the dashboard preview.\n${escapeHtml(error instanceof Error ? error.message : String(error))}</pre>`, 'preview failed');
    }
}
function bindFileLinks() {
    ...
    details.querySelectorAll('a[data-file-link]').forEach(link => {
        link.addEventListener('click', event => {
            ...
            if (!rawHref || rawHref.startsWith('file:'))
                return;
...
function routeList(routes) {
    if (!routes || !routes.length)
        return '<p class="desc">No common route data recorded.</p>';
    return `<ul class="routes">${routes.map(item => `<li><code>${item.route}</code> (${item.count})</li>`).join('')}</ul>`;
}
...
function renderInitDetails(nodes) {
    ...
    document.querySelectorAll('[data-init-id]').forEach(button => {
        button.addEventListener('click', () => selectMemory(button.dataset.initId || ''));
    });
}
function selectMemory(id) {
    state.selectedId = id;
    state.graphMode = 'drilldown';
    document.getElementById('graphMode').value = state.graphMode;
...
function init() {
    ...
    nodeLimit.addEventListener('input', event => {
        state.nodeLimit = Number(event.target.value);
        document.getElementById('nodeLimitLabel').textContent = `${state.nodeLimit} nodes`;
        render();
    });
